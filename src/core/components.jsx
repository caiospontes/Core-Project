...
const LANGUAGE_CYCLE = ['pt-BR', 'es', 'en'];
const LANGUAGE_META = {
...
const UI_TRANSLATIONS = {
  'pt-BR': {
    ...
    lapsReader: 'Leitor de Senha LAPS',
    notebookLabels: 'Etiquetas de Notebooks',
    barcodeLabels: 'Etiquetas (código de barras)',
    dynamicEmission: 'Emissão Dinâmica',
    system: 'Sistema',
  },
  es: {
    ...
    lapsReader: 'Lector de Contraseña LAPS',
    notebookLabels: 'Etiquetas de Portátiles',
    barcodeLabels: 'Etiquetas (código de barras)',
    dynamicEmission: 'Emisión Dinámica',
    system: 'Sistema',
  },
  en: {
    dashboard: 'Home',
    ...
    lapsReader: 'LAPS Password Reader',
    notebookLabels: 'Notebook Labels',
    barcodeLabels: 'Labels (barcode)',
    dynamicEmission: 'Dynamic Issuance',
    system: 'System',
  }
};

const getUiText = (language, key) => UI_TRANSLATIONS[language]?.[key] || UI_TRANSLATIONS['pt-BR'][key] || key;
...
function Dashboard({ user, onLogout, users, setUsers, templates, setTemplates, tagsConfig, setTagsConfig, delimiters, setDelimiters, changelog, setChangelog, toolsConfig, setToolsConfig, systemSettings, cepMappings, correiosPresets, inventoryConfigs }) {
  ...
  const cycleLanguage = () => {
    setLanguage((prev) => {
      ...
      const nextIndex = currentIndex === -1 ? 0 : (currentIndex + 1) % LANGUAGE_CYCLE.length;
        ...
        <nav className="flex-1 overflow-y-auto py-5 custom-scroll">
          <div className="px-3 space-y-1.5">
            <button onClick={() => { setActivePage('Home'); setMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${activePage === 'Home' ? 'bg-gradient-to-r from-cyan-500 to-blue-600 text-white shadow-md shadow-cyan-900/20' : 'hover:bg-slate-800/80 hover:text-white'}`}>
              ...
              {getUiText(language, 'overview')}
            </button>
            
            <div className="pt-2 pb-1">
                  <span className="px-3 text-[10px] font-bold text-slate-500 uppercase tracking-widest">{getUiText(language, 'operations')}</span>
            </div>
            ...
            <div>
              <button onClick={() => toggleMenu('geradores')} className="w-full flex items-center justify-between px-3 py-2 rounded-md text-sm font-medium hover:bg-slate-800 hover:text-white transition-colors">
                <div className="flex items-center gap-3"><svg className="w-5 h-5 opacity-80" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg> {getUiText(language, 'dynamicEmission')}</div>
            ...
            <>
              <div className="pt-4 pb-1">
                  <span className="px-3 text-[10px] font-bold text-slate-500 uppercase tracking-widest">{getUiText(language, 'system')}</span>
              </div>
              <button
        ...
        <header className="h-16 bg-gradient-to-r from-[#002233] to-[#005a77] border-b border-[#00DBFF]/40 px-8 flex items-center justify-between flex-shrink-0 text-white shadow-sm">
            <div className="flex items-center gap-3">
                <div className="hidden md:flex items-center gap-2 text-sm">
                    <span className="text-blue-100/90 font-medium">{activePage === 'Home' ? getUiText(language, 'dashboard') : activePage === 'Admin' ? getUiText(language, 'administration') : activePage === 'LAPS' ? getUiText(language, 'security') : activePage === 'IMEIBarcode' ? getUiText(language, 'operations') : activePage === 'NotebookLabels' ? getUiText(language, 'inventory') : activePage === 'Inventories' ? getUiText(language, 'inventory') : getUiText(language, 'operations')}</span>
                    <span className="text-blue-200">/</span>
                    <span className="font-semibold">{currentTool ? currentTool.label : activePage === 'Home' ? getUiText(language, 'overview') : activePage === 'Admin' ? getUiText(language, 'adminPanel') : activePage === 'LAPS' ? getUiText(language, 'lapsReader') : activePage === 'IMEIBarcode' ? getUiText(language, 'barcodeLabels') : activePage === 'NotebookLabels' ? getUiText(language, 'notebookLabels') : activePage === 'Inventories' ? getUiText(language, 'inventory') : activePage}</span>
                    {activeSubPage && (
                        <>
                            <span className="text-blue-200">/</span>
            ...
            <button
              ...
              className="rounded-full border border-white/30 bg-white/10 hover:bg-white/20 px-3 py-1.5 text-xs font-semibold flex items-center gap-2 transition-colors"
        ...
        <div className="flex-1 overflow-hidden">
            ...
            {activePage === 'Home' && (
              <button
                type="button"
                onClick={cycleLanguage}
                className="absolute top-4 right-6 hidden lg:flex items-center gap-2 text-xs text-[#002233] bg-white/95 border border-[#00DBFF]/40 rounded-full px-3 py-1.5 shadow-sm hover:bg-white transition-colors"
                title="Alternar idioma"
              >
                <span>{LANGUAGE_META[language].flag}</span>
                <span className="font-semibold">{LANGUAGE_META[language].label}</span>
              </button>
            )}
