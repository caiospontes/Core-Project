...
function NotebookInventoryLabelPage({ inventoryConfigs, selectedInventoryId }) {
  ...
  const buildBarcodeValue = (entry) => {
    const compact = [
      normalizeText(entry.status).slice(0, 12),
      normalizeText(entry.marca).slice(0, 12),
      normalizeText(entry.modelo).slice(0, 18),
      normalizeText(entry.processador).slice(0, 16),
      normalizeText(entry.patrimonio).slice(0, 14),
      normalizeText(entry.serviceTag).slice(0, 14),
      normalizeText(entry.observacao).slice(0, 20)
  ...
  const addLabel = (entry) => {
    ...
    const payload = { ...entry, barcodeValue: buildBarcodeValue(entry), id: `${entry.patrimonio}-${entry.serviceTag}-${Date.now()}` };
