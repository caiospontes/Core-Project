...
import { sortTools, getCompositeKey, resolveKeyFromMap, formatDateToBR, getTodayBR, formatDateToISOInput, buildNotebookBarcodeValue, parseNotebookBarcodeValue, buildKeyCandidates } from './utils';
      ...
      <div className="relative w-full max-w-md rounded-[30px] border border-cyan-200/30 bg-[linear-gradient(180deg,rgba(25,50,84,0.86)_0%,rgba(11,24,48,0.92)_100%)] p-8 shadow-[0_25px_80px_-28px_rgba(56,189,248,0.55)] backdrop-blur-xl">
        ...
        <div className="relative text-center mb-7">
          <div className="w-14 h-14 mx-auto rounded-2xl flex items-center justify-center mb-4 border border-cyan-100/40 bg-slate-900/60 shadow-lg shadow-cyan-900/40">
            <img src="https://midias-tdw.totvs.com/wp-content/uploads/2025/06/favicon-bg-light-192x192-1.png" alt="Logo" className="w-9" />
          </div>
          <h1 className="text-3xl font-semibold tracking-tight text-slate-100">Welcome back</h1>
          <p className="text-slate-300/80 text-sm mt-1">Acesse o CORE ERP com suas credenciais.</p>
...
function AdminPanel({ users, templates, tagsConfig, delimiters, changelog, toolsConfig, systemSettings, cepMappings, correiosPresets, inventoryConfigs }) {
  ...
  const activeTagConfigKey = resolveKeyFromMap(tagsConfig, tagModuleFilter, tagSubModuleFilter) || currentTagConfigKey;
  ...
  const handleAddSession = async () => {
    ...
    const currentConfig = tagsConfig[activeTagConfigKey] || { sessions: [] };
    ...
    await setDoc(getDocRef('tags', activeTagConfigKey), updatedConfig);
    setNewSessionName('');
  };

  const handleDeleteSession = async (sessionId) => {
    if (!window.confirm('Excluir sessão e todas as suas tags?')) return;
    const currentConfig = tagsConfig[activeTagConfigKey];
    const updatedSessions = currentConfig.sessions.filter(s => s.id !== sessionId);
    await setDoc(getDocRef('tags', activeTagConfigKey), { ...currentConfig, sessions: updatedSessions });
  };
  ...
  const toggleSessionActive = async (sessionId) => {
      ...
      await setDoc(getDocRef('tags', activeTagConfigKey), { ...currentConfig, sessions });
  };
  ...
  const handleSaveTagPanel = async (e) => {
      e.preventDefault();
      const success = await saveTag(tagForm.id, tagForm.label, tagForm.type, activeTagConfigKey, !!editingTag, editingTag?.tagData.id, tagForm.sessionId, { required: tagForm.required, options: tagForm.options });
      if(success) {
            ...
            {activeTab === 'tags' && (
                <div className="flex flex-col lg:flex-row gap-6 items-start h-full">
                        ...
                        <form onSubmit={handleSaveTagPanel} className="space-y-4">
                            ...
                            <div>
                                <select value={tagForm.sessionId} onChange={e => setTagForm({...tagForm, sessionId: e.target.value})} className="w-full border border-slate-300 p-2 rounded-md text-sm outline-none focus:border-blue-500 bg-white" required>
                                    ...
                                    {(tagsConfig[activeTagConfigKey]?.sessions || []).map(s => <option key={s.id} value={s.id}>{s.title}</option>)}
                                </select>
                            </div>
                            <div>
                     ...
                     <div className="flex-1 space-y-5">
                         {(tagsConfig[activeTagConfigKey]?.sessions || []).map((session, sessionIndex) => (
                             <div 
                                key={session.id} 
...
function Dashboard({ user, onLogout, users, setUsers, templates, setTemplates, tagsConfig, setTagsConfig, delimiters, setDelimiters, changelog, setChangelog, toolsConfig, setToolsConfig, systemSettings, cepMappings, correiosPresets, inventoryConfigs }) {
  ...
  return (
    <div className="flex w-screen h-screen bg-[radial-gradient(circle_at_12%_8%,rgba(56,189,248,0.18)_0,transparent_34%),radial-gradient(circle_at_88%_16%,rgba(59,130,246,0.16)_0,transparent_32%),linear-gradient(165deg,#020817_0%,#071739_45%,#041126_100%)] font-sans text-slate-100 overflow-hidden relative">
      ...
      {/* Sidebar ERP Style */}
      <aside className={`fixed md:relative w-72 bg-[linear-gradient(180deg,#081225_0%,#0b1733_62%,#0a1a3d_100%)] backdrop-blur text-slate-300 flex flex-col flex-shrink-0 z-40 shadow-2xl no-print h-full transition-transform transform border-r border-cyan-400/20 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
      ...
      {/* Main Content Area */}
      <main className="flex-1 flex flex-col h-full overflow-hidden bg-transparent relative text-slate-100">
        
        {/* Dynamic Context Header (Se estiver dentro de um módulo) */}
        {currentTool && (
            <header className="h-16 bg-slate-900/45 backdrop-blur border-b border-cyan-300/15 px-8 flex items-center justify-between flex-shrink-0">
                <div className="flex items-center gap-3">
                    <div className="hidden md:flex items-center gap-2 text-sm">
                        <span className="text-slate-300/70 font-medium">Operações</span>
                        <span className="text-slate-500">/</span>
                        <span className="font-semibold text-slate-100">{currentTool.label}</span>
                        {activeSubPage && (
                            <>
                                <span className="text-slate-500">/</span>
                                <span className="font-bold text-cyan-300">{currentTool.subTypes?.find(s=>s.id === activeSubPage)?.label}</span>
                            </>
                        )}
                    </div>
                </div>
        ...
        <div className="flex-1 overflow-hidden">
            ...
            {activePage === 'Home' && (
              <div className="absolute top-4 right-6 hidden lg:flex items-center gap-2 text-xs text-slate-200 bg-slate-900/65 border border-cyan-300/30 rounded-full px-3 py-1 shadow-sm">
