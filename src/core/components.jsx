...
const CORE_ANALYTICS_KEY = 'core_analytics_events_v1';
const readAnalyticsEvents = () => {
  if (typeof window === 'undefined') return [];
  try {
    const parsed = JSON.parse(localStorage.getItem(CORE_ANALYTICS_KEY) || '[]');
    return Array.isArray(parsed) ? parsed : [];
  } catch {
    return [];
  }
};
const writeAnalyticsEvent = (event) => {
  if (typeof window === 'undefined') return;
  ...
  localStorage.setItem(CORE_ANALYTICS_KEY, JSON.stringify(events.slice(-5000)));
};
...
function AdminPanel({ users, templates, tagsConfig, delimiters, changelog, toolsConfig, systemSettings, cepMappings, correiosPresets, inventoryConfigs }) {
  ...
  const analyticsEvents = useMemo(() => readAnalyticsEvents(), []);
  const periodStart = useMemo(() => {
    const now = new Date();
    if (statsPeriod === 'today') return new Date(now.getFullYear(), now.getMonth(), now.getDate());
    if (statsPeriod === 'week') return new Date(now.getTime() - (7 * 24 * 60 * 60 * 1000));
    return new Date(now.getTime() - (30 * 24 * 60 * 60 * 1000));
  }, [statsPeriod]);
  const filteredEvents = useMemo(() => analyticsEvents.filter((ev) => new Date(ev.ts) >= periodStart), [analyticsEvents, periodStart]);
  const requestEvents = filteredEvents.filter((ev) => ev.type === 'request');
  const uniqueUsers = new Set(requestEvents.map((ev) => ev.userId)).size;
  const moduleCounter = requestEvents.reduce((acc, ev) => {
    const key = ev.module || 'N/D';
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {});
  const topModuleEntry = Object.entries(moduleCounter).sort((a, b) => b[1] - a[1])[0] || ['N/D', 0];
  const hourCounter = requestEvents.reduce((acc, ev) => {
    const hour = new Date(ev.ts).getHours();
    acc[hour] = (acc[hour] || 0) + 1;
    return acc;
  }, {});
  const peakHour = Object.entries(hourCounter).sort((a, b) => b[1] - a[1])[0];
  const groupCounter = requestEvents.reduce((acc, ev) => {
    const key = ev.group || 'N/D';
    acc[key] = (acc[key] || 0) + 1;
    return acc;
  }, {});
  const topGroups = Object.entries(groupCounter).sort((a, b) => b[1] - a[1]).slice(0, 3);
  const minutesSaved = requestEvents.reduce((acc, ev) => acc + Number(ev.minutesSaved || 0), 0);
  const hoursSaved = (minutesSaved / 60).toFixed(1);
  const successCount = requestEvents.filter((ev) => ev.status === 'success').length;
  const successRate = requestEvents.length > 0 ? ((successCount / requestEvents.length) * 100).toFixed(1) : '100.0';
  const moduleChartData = Object.entries(moduleCounter).sort((a, b) => b[1] - a[1]).slice(0, 6);
  const maxModuleHits = Math.max(...moduleChartData.map(([, count]) => count), 1);
  const hourChartData = Object.entries(hourCounter).sort((a, b) => Number(a[0]) - Number(b[0]));
  const maxHourHits = Math.max(...hourChartData.map(([, count]) => count), 1);
    ...
    <div className="flex h-full w-full bg-slate-50 overflow-hidden">
      <div className="w-64 bg-white border-r border-slate-200 flex-shrink-0 flex flex-col no-print z-10 shadow-sm">
        ...
        <nav className="flex-1 py-4 space-y-1 overflow-y-auto">
            {['templates', 'tags', 'backup', 'tools', 'inventories', 'integrations', 'correios', 'users', 'changelog', 'stats', 'config'].map(tab => (
                <button 
                    key={tab} 
                ...
                <div className="space-y-6">
                    <div className="bg-white p-6 rounded-lg shadow-sm border border-slate-200">
                        <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
                            <h3 className="font-semibold text-lg text-slate-900">Relatório Executivo de Impacto e Engajamento</h3>
                            <select value={statsPeriod} onChange={(e) => setStatsPeriod(e.target.value)} className="border border-slate-300 rounded-md px-3 py-2 text-sm bg-white">
                                <option value="today">Hoje</option>
                                <option value="week">Esta semana</option>
                                <option value="month">Este mês</option>
                            </select>
                        </div>
                        <p className="text-sm text-slate-700 bg-blue-50 border border-blue-200 rounded-md p-4">
                            <strong>Resumo Executivo (TL;DR):</strong> no período selecionado, o CORE registrou <strong>{requestEvents.length}</strong> acessos com <strong>{successRate}%</strong> de sucesso, envolvendo <strong>{uniqueUsers}</strong> usuários únicos ativos. O principal destaque foi o módulo <strong>{topModuleEntry[0]}</strong>, com <strong>{topModuleEntry[1]}</strong> acionamentos.
                        </p>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-white p-5 rounded-lg shadow-sm border border-slate-200">
                            <h4 className="font-semibold text-slate-900 mb-2">Visão Geral de Acessos</h4>
                            <ul className="text-sm text-slate-700 space-y-1">
                                <li>• Volume total de acessos: <strong>{requestEvents.length}</strong></li>
                                <li>• Usuários únicos ativos: <strong>{uniqueUsers}</strong></li>
                            </ul>
                        </div>

                        <div className="bg-white p-5 rounded-lg shadow-sm border border-slate-200">
                            <h4 className="font-semibold text-slate-900 mb-2">Engajamento por Módulo (Campeões)</h4>
                            <ul className="text-sm text-slate-700 space-y-1">
                                <li>• Módulo mais utilizado: <strong>{topModuleEntry[0]}</strong></li>
                                <li>• Quantidade de acionamentos: <strong>{topModuleEntry[1]}</strong></li>
                                <li>• Insight de negócio: indica automação de tarefas críticas e redução de gargalos operacionais recorrentes.</li>
                            </ul>
                        </div>

                        <div className="bg-white p-5 rounded-lg shadow-sm border border-slate-200">
                            <h4 className="font-semibold text-slate-900 mb-2">Comportamento e Adoção</h4>
                            <ul className="text-sm text-slate-700 space-y-1">
                                <li>• Horário de pico: <strong>{peakHour ? `${peakHour[0]}h (${peakHour[1]} acessos)` : 'N/D'}</strong></li>
                                <li>• Grupos/departamentos mais engajados:</li>
                                {topGroups.length > 0 ? topGroups.map(([group, count]) => (
                                    <li key={group}>- <strong>{group}</strong>: {count} acessos</li>
                                )) : (
                                    <li>- N/D</li>
                                )}
                            </ul>
                        ...
                        <div className="bg-white p-5 rounded-lg shadow-sm border border-slate-200">
                            <h4 className="font-semibold text-slate-900 mb-2">Impacto Real / Horas Salvas (Estimativa)</h4>
                            <ul className="text-sm text-slate-700 space-y-1">
                                <li>• Minutos economizados no período: <strong>{minutesSaved}</strong></li>
                                <li>• Horas redirecionadas para tarefas estratégicas: <strong>{hoursSaved}h</strong></li>
                            </ul>
                        </div>
                    </div>

                    <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
                        <h4 className="font-semibold text-slate-900 mb-1">Saúde do Sistema</h4>
                        <p className="text-sm text-slate-700">Taxa de sucesso das requisições: <strong>{successRate}%</strong> ({successCount}/{requestEvents.length || 0}).</p>
                    </div>

                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                        <div className="bg-white p-5 rounded-lg shadow-sm border border-slate-200">
                            <h4 className="font-semibold text-slate-900 mb-4">Gráfico — Uso por Módulo</h4>
                            <div className="space-y-3">
                                {moduleChartData.length > 0 ? moduleChartData.map(([moduleName, count]) => (
                                    <div key={moduleName}>
                                        <div className="flex items-center justify-between text-xs text-slate-600 mb-1">
                                            <span className="truncate max-w-[80%]">{moduleName}</span>
                                            <strong>{count}</strong>
                                        </div>
                                        <div className="h-2.5 rounded-full bg-slate-100 overflow-hidden">
                                            <div className="h-full rounded-full bg-gradient-to-r from-cyan-500 to-blue-600" style={{ width: `${(count / maxModuleHits) * 100}%` }} />
                                        </div>
                                    </div>
                                )) : <p className="text-sm text-slate-400">Sem dados para o período.</p>}
                            </div>
                        </div>

                        <div className="bg-white p-5 rounded-lg shadow-sm border border-slate-200">
                            <h4 className="font-semibold text-slate-900 mb-4">Gráfico — Distribuição por Hora</h4>
                            <div className="grid grid-cols-6 gap-2">
                                {hourChartData.length > 0 ? hourChartData.map(([hour, count]) => (
                                    <div key={hour} className="flex flex-col items-center justify-end gap-1">
                                        <div className="w-full h-24 flex items-end">
                                            <div className="w-full rounded-t bg-gradient-to-t from-violet-500 to-cyan-400" style={{ height: `${Math.max((count / maxHourHits) * 100, 6)}%` }} />
                                        </div>
                                        <span className="text-[10px] text-slate-500">{hour}h</span>
                                    </div>
                                )) : <p className="text-sm text-slate-400 col-span-6">Sem dados para o período.</p>}
                            </div>
                        </div>
                    </div>
                </div>
...
function Dashboard({ user, onLogout, users, setUsers, templates, setTemplates, tagsConfig, setTagsConfig, delimiters, setDelimiters, changelog, setChangelog, toolsConfig, setToolsConfig, systemSettings, cepMappings, correiosPresets, inventoryConfigs }) {
  ...
  const filteredTools = sortTools(toolsConfig).filter(([key, tool]) =>
    !normalizedSearch
    || tool.label.toLowerCase().includes(normalizedSearch)
    || key.toLowerCase().includes(normalizedSearch)
  );
  const filteredInventories = (inventoryConfigs && inventoryConfigs.length > 0 ? inventoryConfigs : DEFAULT_INVENTORY_CONFIGS).filter((inventory) =>
    !normalizedSearch || inventory.name.toLowerCase().includes(normalizedSearch)
  );

  useEffect(() => {
    const moduleLabel = currentTool?.label
      || (activePage === 'Home' ? 'Visão Geral'
      : activePage === 'Admin' ? 'Painel Administrativo'
      : activePage === 'LAPS' ? 'Leitor de Senha LAPS'
      : activePage === 'IMEIBarcode' ? 'Etiquetas (código de barras)'
      : activePage === 'NotebookLabels' ? 'Etiquetas de Notebooks'
      : activePage === 'Inventories' ? 'Inventários'
      : activePage);
    const minutesMap = {
      Home: 1,
      Admin: 3,
    ...
    writeAnalyticsEvent({
      ...
      module: moduleLabel,
      page: activePage,
      subPage: activeSubPage || null,
      userId: user?.email || user?.id || user?.name || 'anonymous',
      group: user?.role || 'operador',
      minutesSaved: minutesMap[activePage] ?? 5
    });
  }, [activePage, activeSubPage, currentTool, user]);
          ...
          <div className="px-3 space-y-1.5">
            <div className="px-1 pb-2">
              <div className="flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-900/60 px-3 py-2">
                <svg className="w-4 h-4 text-slate-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-4.35-4.35m1.85-4.65a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                <input
                  value={searchQuery}
