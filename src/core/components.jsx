...
import { sortTools, getCompositeKey, resolveKeyFromMap, formatDateToBR, getTodayBR, formatDateToISOInput, buildNotebookBarcodeValue, parseNotebookBarcodeValue } from './utils';
...
function SafePreview({ html }) {
  ...
  const parsedPreview = useMemo(() => {
    ...
    if (!hasFullHtml || typeof window === 'undefined') {
      return { isFullHtml: false, docStyles: '', bodyContent: sanitizeHtml(html || '') };
    }

    const parser = new DOMParser();
    const doc = parser.parseFromString(html || '', 'text/html');
    const docStyles = Array.from(doc.head.querySelectorAll('style')).map((s) => s.textContent || '').join('\n');

    return { isFullHtml: true, docStyles, bodyContent: sanitizeHtml(doc.body.innerHTML || '') };
  }, [html]);
...
function LoginPage({ onLogin, users, dbReady, systemSettings }) {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-[radial-gradient(circle_at_15%_20%,#c7d2fe_0%,transparent_35%),radial-gradient(circle_at_85%_15%,#bfdbfe_0%,transparent_35%),linear-gradient(160deg,#eef2ff_0%,#f8fbff_45%,#eff6ff_100%)] px-4 py-8 font-sans text-slate-800">
      <div className="glass-panel w-full max-w-md p-8 rounded-3xl border border-indigo-100/80">
        
        <div className="text-center mb-8">
          <div className="w-16 h-16 mx-auto rounded-xl flex items-center justify-center mb-4 border border-indigo-100 shadow-sm bg-gradient-to-br from-white to-indigo-50">
        ...
        <div className="space-y-6">
          <button
            ...
            disabled={loading}
            className="w-full bg-white border border-indigo-100 text-slate-700 font-semibold py-2.5 rounded-xl hover:bg-indigo-50/40 transition-all hover:shadow-md flex items-center justify-center gap-3 shadow-sm"
          >
            {loading ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button type="submit" disabled={loading} className="w-full bg-gradient-to-r from-indigo-600 via-blue-600 to-cyan-600 text-white font-semibold py-2.5 rounded-xl hover:from-indigo-700 hover:via-blue-700 hover:to-cyan-700 transition-all shadow-lg shadow-blue-500/20 text-sm mt-2">
              Entrar no Sistema
            </button>
...
function HomePage({ onNavigate, user, changelog, toolsConfig }) {
  ...
  return (
  <div className="h-full w-full flex flex-col bg-[linear-gradient(160deg,#f8fbff_0%,#eef6ff_45%,#eef2ff_100%)] overflow-y-auto">
    <div className="bg-white/85 backdrop-blur border-b border-indigo-100 px-8 py-6 sticky top-0 z-10">
    ...
    <div className="flex-1 p-8 max-w-6xl mx-auto w-full space-y-8">
      <section>
        <h2 className="text-sm font-semibold text-slate-800 uppercase tracking-wider mb-4">Atalhos rápidos</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {quickShortcuts.map((shortcut) => (
            <button
              ...
              className={`text-left rounded-2xl border p-5 transition-all ${shortcut.restricted ? 'bg-slate-100/80 border-slate-200 cursor-not-allowed opacity-70' : 'bg-white border-indigo-100 hover:-translate-y-0.5 hover:shadow-xl hover:border-indigo-300'}`}
              title={shortcut.restricted ? 'Acesso restrito a administradores' : shortcut.label}
            >
              <div className="flex items-start gap-3">
                <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${shortcut.tone} text-white flex items-center justify-center shadow-sm`}>
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-500 mt-1">{shortcut.desc}</p>
                </div>
              </div>
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-8">
        <section>
          <h2 className="text-sm font-semibold text-slate-800 uppercase tracking-wider mb-4">Todos os módulos</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {sortTools(toolsConfig).map(([key, tool]) => (
                  <div key={key} onClick={() => tool.active && onNavigate(key)} className={`bg-white p-5 rounded-2xl shadow-sm border border-indigo-100 transition-all duration-200 ${tool.active ? 'hover:-translate-y-1 hover:border-indigo-300 hover:shadow-xl hover:shadow-indigo-100/70 cursor-pointer' : 'opacity-60 cursor-not-allowed bg-slate-50'}`}>
                      <div className="flex items-start gap-4">
                          <div className="w-10 h-10 bg-gradient-to-br from-indigo-50 to-cyan-50 text-indigo-600 rounded-xl flex-shrink-0 flex items-center justify-center border border-indigo-100">
                          ...
                          <div>
                              ...
                              {tool.subTypes && tool.subTypes.length > 0 && <span className="mt-1 text-[10px] bg-indigo-50 border border-indigo-200 text-indigo-700 font-semibold px-2 py-0.5 rounded inline-block">{tool.subTypes.length} variações</span>}
        ...
        <section>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-indigo-100 p-0 overflow-hidden">
...
function AdminPanel({ users, templates, tagsConfig, delimiters, changelog, toolsConfig, systemSettings, cepMappings, correiosPresets, inventoryConfigs }) {
  ...
  const handleEditTemplate = (moduleKey, subKey = '') => { 
      ...
      const resolvedTemplateKey = resolveKeyFromMap(templates, moduleKey, subKey) || key;
  ...
  const handleSaveTagPanel = async (e) => {
      e.preventDefault();
      const success = await saveTag(tagForm.id, tagForm.label, tagForm.type, currentTagConfigKey, !!editingTag, editingTag?.tagData.id, tagForm.sessionId, { required: tagForm.required, options: tagForm.options });
      if(success) {
    ...
    <div className="flex h-full w-full bg-slate-50 overflow-hidden">
      <div className="w-64 bg-white border-r border-slate-200 flex-shrink-0 flex flex-col no-print z-10 shadow-sm">
        ...
        <nav className="flex-1 py-4 space-y-1 overflow-y-auto">
            {['templates', 'tags', 'tools', 'inventories', 'integrations', 'correios', 'users', 'changelog', 'config'].map(tab => (
                <button 
                    key={tab} 
...
function DynamicGenerator({ template, tagsConfig, delimiters, moduleId, cepMappings, correiosPresets = [] }) {
  ...
  useEffect(() => {
    const resolvedModuleKey = resolveKeyFromMap(tagsConfig, moduleId) || resolveKeyFromMap(tagsConfig, baseModuleId);
  ...
  const renderTagInput = (tag) => {
    ...
    <div key={tag.id} className={`bg-white p-3 rounded-md border shadow-sm focus-within:border-blue-400 focus-within:ring-1 focus-within:ring-blue-400 transition-all ${requiredMissing ? 'border-red-300 bg-red-50/40' : 'border-slate-200'}`}>
      ...
      <p className="text-[10px] text-slate-400 mb-2">{getTagBindingHint(tag)}</p>
  ...
  const handlePrint = () => {
      const printContent = sanitizeHtml(renderDocument());
      ...
      <div className="w-full md:w-[420px] bg-white border-r border-slate-200 flex flex-col z-10 no-print shadow-sm">
        ...
        <div className="flex-1 overflow-y-auto p-6 custom-scroll space-y-8 bg-slate-50/50">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-800 space-y-1">
              ...
              <p>• A linha “Tag vinculada” mostra exatamente onde o valor entra no template.</p>
...
function Dashboard({ user, onLogout, users, setUsers, templates, setTemplates, tagsConfig, setTagsConfig, delimiters, setDelimiters, changelog, setChangelog, toolsConfig, setToolsConfig, systemSettings, cepMappings, correiosPresets, inventoryConfigs }) {
  ...
  return (
    <div className="flex w-screen h-screen bg-[radial-gradient(circle_at_12%_8%,rgba(59,130,246,0.14)_0,transparent_38%),radial-gradient(circle_at_82%_18%,rgba(139,92,246,0.16)_0,transparent_34%),linear-gradient(145deg,#f3f7ff_0%,#edf4ff_35%,#eaf3ff_100%)] font-sans text-slate-800 overflow-hidden relative">
      ...
      {/* Mobile Toggle */}
      <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="md:hidden absolute top-4 left-4 z-50 text-white bg-gradient-to-r from-indigo-600 to-blue-600 p-2 rounded-xl shadow-lg border border-indigo-400/50">
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 12h16M4 18h16" /></svg>
      </button>

      {/* Sidebar ERP Style */}
      <aside className={`fixed md:relative w-72 bg-[linear-gradient(180deg,#0f172a_0%,#111b36_60%,#132042_100%)] backdrop-blur text-slate-300 flex flex-col flex-shrink-0 z-40 shadow-2xl no-print h-full transition-transform transform ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <nav className="flex-1 overflow-y-auto py-5 custom-scroll">
          <div className="px-3 space-y-1.5">
            <button onClick={() => { setActivePage('Home'); setMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-all ${activePage === 'Home' ? 'bg-gradient-to-r from-indigo-600 to-blue-600 text-white shadow-md shadow-blue-900/20' : 'hover:bg-slate-800/80 hover:text-white'}`}>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden bg-transparent relative">
        ...
        {currentTool && (
            <header className="h-16 bg-white/75 backdrop-blur border-b border-indigo-100 px-8 flex items-center justify-between flex-shrink-0">
                <div className="flex items-center gap-3">
        ...
        <div className="flex-1 overflow-hidden">
            ...
            {currentTool && (!currentTool?.subTypes?.length || activeSubPage) && (
                <DynamicGenerator 
                    template={templates[resolveKeyFromMap(templates, activePage, activeSubPage) || getCompositeKey(activePage, activeSubPage)]} 
