...
import { DEFAULT_USERS, DEFAULT_TAGS_WITH_SESSIONS, DEFAULT_TOOLS_CONFIG, DEFAULT_CHANGELOG, DEFAULT_HTML_TEMPLATE } from './defaults';
...
function LapsReaderPage() {
  ...
  const tokens = rawPassword.split('').map((ch) => {
    ...
    return { char: ch, spoken: `${ch} ${label}` };
  });
    ...
    <div className="h-full overflow-y-auto custom-scroll bg-gradient-to-b from-slate-50/90 to-slate-100/60 p-6 md:p-10">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="mt-6 bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          ...
          <div className="min-h-[148px] rounded-xl border border-slate-200 bg-slate-50/80 p-4 text-2xl text-slate-800 leading-relaxed break-words">
            {spokenPassword || 'A leitura da senha aparecerá aqui no formato: caractere + descrição.'}
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2">
              {tokens.map((token, idx) => (
                <div key={`${token.char}-${idx}`} className="rounded-lg border border-slate-200 px-3 py-2 text-sm bg-slate-50/80">
                  ...
                  <span className="font-medium text-blue-700">{token.spoken.replace(`${token.char} `, '')}</span>
                </div>
              ))}
            </div>
          </div>
