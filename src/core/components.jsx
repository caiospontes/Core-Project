...
import { sortTools, getCompositeKey } from './utils';
...
function ImeiBarcodePage() {
  ...
  return (
    <div className="h-full overflow-y-auto custom-scroll bg-gradient-to-b from-slate-50/90 to-slate-100/60 p-6 md:p-8">
      <style>{`@media print { .imei-no-print { display: none !important; } .imei-print-grid { grid-template-columns: repeat(3, minmax(0, 1fr)) !important; gap: 10px !important; } body { background: white !important; } }`}</style>
      <div className="max-w-7xl mx-auto space-y-6">
        <div>
            ...
            {imeis.length === 0 ? (
              ...
              <div className="imei-print-grid grid grid-cols-1 sm:grid-cols-2 gap-4">
                {imeis.map((imei) => {
                  ...
                  return (
                    <div key={imei} className="border border-slate-300 rounded-lg p-3 bg-white break-inside-avoid">
                      ...
                      <svg width="100%" viewBox={`0 0 ${barcode.width} ${barcode.height}`} preserveAspectRatio="none" className="h-16 bg-white">
                        {barcode.bars.map((bar, idx) => (
                          <rect key={`${imei}-bar-${idx}`} x={bar.x} y="0" width={bar.width} height={barcode.height} fill="#111827" />
                        ))}
                      </svg>
                      <p className="text-center font-mono text-xs tracking-wider text-slate-800 mt-2">{imei}</p>
                    </div>
                  );
                })}
              </div>
...
function Dashboard({ user, onLogout, users, setUsers, templates, setTemplates, tagsConfig, setTagsConfig, delimiters, setDelimiters, changelog, setChangelog, toolsConfig, setToolsConfig, systemSettings, cepMappings, inventoryConfigs }) {
  ...
  const currentTool = activePage !== 'Home' && activePage !== 'Admin' && activePage !== 'LAPS' && activePage !== 'Inventories' && activePage !== 'IMEIBarcode' ? toolsConfig[activePage] : null;
