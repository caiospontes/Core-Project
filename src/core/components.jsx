...
function AdminPanel({ users, templates, tagsConfig, delimiters, changelog, toolsConfig, systemSettings, cepMappings, correiosPresets, inventoryConfigs }) {
  ...
  const mapTemplateTagsToDictionary = async (moduleKey, subKey = '', htmlOverride = null) => {
    ...
    const resolvedTagConfigKey = resolveKeyFromMapWithLegacyPrefix(tagsConfig, moduleKey, subKey) || compositeKey;
    const currentConfig = normalizeTagDoc(tagsConfig[resolvedTagConfigKey]);
    ...
    const existingIds = new Set(firstSession.tags.map((tag) => String(tag.id || '').toUpperCase()));
    let addedCount = 0;
    ...
    await setDoc(getDocRef('tags', resolvedTagConfigKey), { ...currentConfig, sessions });
...
function Dashboard({ user, onLogout, users, setUsers, templates, setTemplates, tagsConfig, setTagsConfig, delimiters, setDelimiters, changelog, setChangelog, toolsConfig, setToolsConfig, systemSettings, cepMappings, correiosPresets, inventoryConfigs }) {
  ...
  return (
    <div className="flex w-screen h-screen bg-[radial-gradient(circle_at_12%_8%,rgba(59,130,246,0.12)_0,transparent_34%),radial-gradient(circle_at_88%_16%,rgba(37,99,235,0.12)_0,transparent_32%),linear-gradient(165deg,#eef4ff_0%,#e7efff_45%,#e4ecfb_100%)] font-sans text-slate-800 overflow-hidden relative">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden bg-transparent relative text-slate-800">
        ...
        <header className="h-16 bg-gradient-to-r from-blue-900 to-blue-700 border-b border-blue-800 px-8 flex items-center justify-between flex-shrink-0 text-white shadow-sm">
            <div className="flex items-center gap-3">
                <div className="hidden md:flex items-center gap-2 text-sm">
                    ...
                    <span className="font-semibold">{currentTool ? currentTool.label : activePage === 'Home' ? 'Visão Geral' : activePage === 'Admin' ? 'Painel Administrativo' : activePage === 'LAPS' ? 'Leitor de Senha LAPS' : activePage === 'IMEIBarcode' ? 'IMEI para Código de Barras' : activePage === 'NotebookLabels' ? 'Etiquetas de Notebooks' : activePage === 'Inventories' ? 'Inventários' : activePage}</span>
                    {activeSubPage && (
                        <>
                            <span className="text-blue-200">/</span>
        ...
        <div className="flex-1 overflow-hidden">
            ...
            {activePage === 'Home' && (
              <div className="absolute top-4 right-6 hidden lg:flex items-center gap-2 text-xs text-blue-900 bg-white/90 border border-blue-200 rounded-full px-3 py-1 shadow-sm">
